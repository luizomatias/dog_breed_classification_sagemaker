...
def test(model, test_loader, epoch, hook, device, criterion):
...
def main(args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        model=train(model, train_data_loader, loss_criterion, optimizer, epoch, hook, device)
        logger.info(f"Epoch Testing: {epoch}")
        test(model, test_data_loader, epoch, hook, device, loss_criterion)
